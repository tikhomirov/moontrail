...
**Полное описание:** MoonTrail — это полноценный слой аудита для админ-панелей MoonShine. Пакет расширяет `spatie/laravel-activitylog`: добавляет структурированную историю активности, построчный side-by-side diff, снапшоты версий модели и транзакционный rollback с полной трассируемостью.
...
Это включает:
- Автоматическое логирование через Spatie
...
Автор глубоко увлечён Севером и его эпосами — ненецкой, селькупской, нганасанской мифологией — и этот образ идеально лёг на аудит-лог. **MoonTrail** видит всё: удалённые записи, перезаписанные поля, откаченные изменения. От лунного следа не скрыться.
