 ...
 */
final class DatabaseActivityQuery implements ActivityQueryContract
{
    /**
     * @param array<string, mixed> $filters
     * @return LengthAwarePaginator<int, MoonTrailActivity>
     */
    ...
    {
        $activity = MoonTrailActivity::query()->with(['subject', 'causer'])->find($id);

        return $activity instanceof MoonTrailActivity ? new DatabaseActivityAdapter($activity) : null;
    }

    public function stats(): array
    {
        /** @var array<string, int> $eventCounts */
        $eventCounts = MoonTrailActivity::query()
    ...
    public function distinctValues(string $column): array
    {
        ...
        $query = MoonTrailActivity::query();
        $table = $query->getModel()->getTable();
        $this->warnDistinctValuesIfLarge($query->getModel(), $column);
        ...
        if ($column === 'subject_type' && Schema::hasColumn($table, 'model_type')) {
            /** @var list<string> $legacy */
            $legacy = MoonTrailActivity::query()
                ->distinct()
                ...
                ->values()
                ->all();
            $values = array_values(array_unique(array_merge($values, $legacy)));
        }
