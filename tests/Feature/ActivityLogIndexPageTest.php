...
it('resolveFilterValue returns direct query param', function (): void {
    $request = request()->replace(['event' => 'updated']);
    app()->instance('request', $request);

    $page = app(MoonTrailIndexPage::class);

    /** @var string|null $value */
    $value = callProtected($page, 'resolveFilterValue', ['event']);

    expect($value)->toBe('updated');
});

it('resolveFilterValue returns nested filters param', function (): void {
    $request = request()->replace(['filters' => ['event' => 'deleted']]);
    app()->instance('request', $request);
...
it('resolveFilterValue returns null when param absent', function (): void {
...
it('buildRemoveFilterUrl strips only the given key from query string', function (): void {
    ...
    /** @var string $url */
    $url = callProtected($page, 'buildRemoveFilterUrl', ['event']);

    expect($url)->toContain('log_name=default')
        ->not->toContain('event=');
});

it('buildClearAllFiltersUrl removes all filter keys', function (): void {
    $request = request()->create('http://localhost/admin/moontrail', 'GET', [
        'event'        => 'created',
        'log_name'     => 'default',
    ...
    /** @var string $url */
    $url = callProtected($page, 'buildClearAllFiltersUrl');
