...
it('resolves custom driver and uses configured activity model', function (): void {
    ...
    expect($runtime->configuredDriver)->toBe('custom')
        ...
        ->and($runtime->activityModel)->toBe(TestCustomActivity::class)
...
it('throws for invalid activity logger driver', function (): void {
    ...
    expect(static fn () => app(ActivityRuntimeResolver::class)->resolve())
        ->toThrow(RuntimeException::class, 'Unsupported moontrail.activity_logger driver: weird-driver');
});
